...
void OrientationSystem::onFrame(SceneManager& scene)
{
	...
	auto& output = scene.getPool<TextUI>().getComponent(2);
	auto& output2 = scene.getPool<TextUI>().getComponent(3);
	output2.position = { 10,30 };
	...
	for (auto& [entID, tr] : _transforms)
	{
		...
		output2.content = 
			std::to_wstring(cameraOr.r[3].m128_f32[0]) + L" " +
			std::to_wstring(cameraOr.r[3].m128_f32[1]) + L" " +
			std::to_wstring(cameraOr.r[3].m128_f32[2]) + L" " +
			std::to_wstring(cameraOr.r[3].m128_f32[3]);
	}
